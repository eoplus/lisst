...
# TODO:
# (1) NOW THE LISST_REG FUNCTION IS WRITING TO R/LISST_INST.RDA, BUT SHOULD WRITE TO PACKAGE AREA,
#     SO THAT CAN BE LOADED FOR ANY SECTION IN ANY WORKING DIRECTORY.
# (2) GET ANGLES FOR LISST-200X VSF.
#

#
# Object: .LISSTm
#
# A list of model dependent parameters list for each supported model.
#
# Variables:
...
# s0    - Minimum size range (µm) per inversion type (ss or rs) and per detector type (model dependent).
# lvarn - Long variable names.
# varun - Variable units.
# aang  - In air angles (rad) for each detector ring (assinged to the correct bins), per detector type (model dependent).
# wang  - In water angles (rad) for each detector ring(assinged to the correct bins), per detector type (model dependent).
# binr  - Bin size range (µm) per inversion type (ss or rs) and per detector type (model dependent).
#
...
.LISSTm <- list(
	"100"  = list(
		...
		varun = list(ud_units$mW, ud_units$V, ud_units$V, ud_units$mW, ud_units$m, ud_units$`°C`, unitless, unitless, unitless, ud_units$m^-1),
		wang = list(
	...
	"200"  = list(
		...
		lvarn = c(paste("Bin", formatC(1:36, width = 2, flag = 0), sep = "_"),"Laser transmission","Battery voltage","External input 1","Laser Reference","Depth","Temperature","Year","Month","Day","Hour","Minute","Second","External input 2","Mean Diameter","Total Volume Concentration","Relative Humidity","Raw pressure 1","Raw pressure 2","Ambient Light","Optical transmission","Beam attenuation"),
		varun = list(ud_units$mW, ud_units$V, ud_units$V, ud_units$mW, ud_units$m, ud_units$`°C`, unitless, unitless, unitless, ud_units$hr, ud_units$min, ud_units$s, ud_units$V, ud_units$µm, ud_units$ppm, ud_units$`%`, unitless, unitless, unitless, unitless, ud_units$m^-1),
		wang = list(
			A = NULL
		),
		binr = list(
...
# Object: .LISSTi
#
# A list of registered LISST instruments.
#
...
# Function: lisst_reg
#
# Register a LISST instrument for import of processing parameters.
#
# Input:
# model - The model of the LISST instrument (e.g., "100x" for the LISST-100x).
# path  - Path to folder containing instrument instalation files. See details.
#
# Ouput:
# Adds a new element to the lisst_int list of registered instruments.
#
# Details:
# The function expects that file naming convention for LISST installation files. Specifically,
# it will search for a factory background file (factory_zsc_*.asc), a ring area file 
# (ringarea_*.asc), an instrument data file (InstrumentData.txt) and a LISST SOP ini file 
# (lisst.ini). All files should be in the same directory provided in the path argument, and it
# should contain files for a single instrument.
#
...
lisst_reg <- function(model, path) {
	...
	if(model == "100") {
		...
		.LISSTi[[as.character(dat[1])]] <- list(
			...
			fzscat = zsc,
			ringcf = rig,
			ringcc = set_units(5, V) / 4096 / set_units(dat[, 3], V/W),
			volcc  = dat[, 4],
		...
		save(".LISSTi", file = "R/lisst_inst.rda")
		assign(".LISSTi", .LISSTi, envir = .GlobalEnv)
