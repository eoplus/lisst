...
attenuation are corrected to add effect of pure water. The same process is 
performed in the rings digital counts for the binary data.
...
dependent on lisst object type. The units can also modified, used in the axis
can also be modified, but note that unit conversion is performed automaticly 
by the units package, such that if the original data units cannot be converted 
into the requested units, it will generate an error. Here is an example on the 
visualization of the particle VSF and the particle approximate phase function, 
that is, the VSF normalized to the particle attenuation (water effect removed)
instead of particle scattering:
